...
class LitAutoEncoder(L.LightningModule):
    ...
    def forward(self, x):
        ...
        return x
        # Inside the forward method of LitAutoEncoder clas
    ...
    def configure_optimizers(self):
        # Define your optimizer and optionally learning rate schedulers
        optimizer = torch.optim.Adam(self.parameters(), lr=1e-3)
        return optimizer
...
model = LitAutoEncoder(input_size, hidden_size)
...
for epoch in range(trainer.max_epochs):
    ...
    for batch_idx, (input_data, target_data) in enumerate(train_loader):
        # Flatten the input_data
        input_data = input_data.view(input_data.size(0), -1)

          # Convert target data to Float
        target_data = target_data.float()  # Or target_data = target_data.long() for classification
    ...
    # Validation phase
    for batch_idx, (input_data, target_data) in enumerate(valid_loader):
        # Flatten the input_data
        input_data = input_data.view(input_data.size(0), -1)
    ...
    # Print epoch summary
    print(f"Epoch [{epoch+1}/{trainer.max_epochs}], Train Loss: {loss.item()}, Validation Loss: {val_loss.item()}")

# Print a message after finishing the training loop
print("Training loop finished.")

#Train with the validation loop (B had lots of errors)
#from torch.utils.data import DataLoader

#train_loader = DataLoader(train_set)
#valid_loader = DataLoader(valid_set)

#input_size = 784 # Assuming you have 10 features in your dataset
#hidden_size = 64 # For example, you can choose a hidden size of 4
...
trainer = Trainer(default_root_dir="/content/drive/My Drive/Colab Notebooks/Checkpoints")
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename='checkpoint_{epoch}-{val_loss:.2f}',
    monitor='val_loss',
    mode='min',
    save_top_k=3
)
...
trainer = Trainer(
    ...
    default_root_dir="/content/drive/My Drive/Colab Notebooks/Checkpoints"
)

model = MyLightningModule.load_from_checkpoint("/content/drive/My Drive/Colab Notebooks/Checkpoints")
...
#par example Let’s use the AutoEncoder as a feature extractor in a separate MODEL
